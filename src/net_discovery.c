...
void *discovery_receiver_listen(void *data){
    ...
    discovery_receiver_t *discovery_receiver = (discovery_receiver_t*)data;
...
void discovery_transmitter_ping(discovery_transmitter_t *discovery_transmitter){
    ...
    for(int port = port_remote_min; port <= port_remote_max; port++){
        ...
        do{
            ...
            received_bytes = p_socket_receive_from(discovery_transmitter->socket, &remote_address, buffer, buffer_read_size, NULL);
            if(received_bytes == 0){
                continue;
            }
